...
if (npc.getPlayerLevel() < 20) {
	...
	if (approve == 0) {
		...
		if (!npc.playerHasMesos(cost[selection]) || !npc.playerCanHoldItem(items[selection], 1)) {
			npc.say("Are you lacking mesos? Check and see if you have an empty slot on your etc. inventory or not.");
		} else {
			npc.takeMesos(cost[selection]);
			npc.giveItem(items[selection], 1);
