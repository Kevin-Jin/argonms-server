...
public class Trade extends Miniroom {
	...
	private boolean canFitAllItemsAndMesos() {
		...
		for (byte i = 0; i < getMaxPlayers(); i++) {
			Player p = getPlayerByPosition(i);
	...
	private void giveItemsAndMesos(Player to, byte from, boolean taxAndShowGain) {
		if (taxAndShowGain)
		...
		for (InventorySlot item : items[from]) {
			if (item != null) {
				...
				for (Short s : changedSlots.modifiedSlots) {
					pos = s.shortValue();
					slot = inv.get(pos);
					to.getClient().getSession().send(CommonPackets.writeInventorySlotUpdate(type, pos, slot));
				}
				for (Short s : changedSlots.addedOrRemovedSlots) {
					pos = s.shortValue();
					slot = inv.get(pos);
					to.getClient().getSession().send(CommonPackets.writeInventoryAddSlot(type, pos, slot));
				}
				if (taxAndShowGain)
					to.getClient().getSession().send(CommonPackets.writeShowItemGain(item.getDataId(), item.getQuantity()));
			}
		}
	}
	...
	public void addItem(Player p, byte slot, InventorySlot item) {
	...
	public void addMesos(Player p, int gain) {
		byte pos = positionOf(p);
