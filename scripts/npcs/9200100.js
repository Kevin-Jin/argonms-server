...
function getRandomColor(currentFace) {
	let style = currentFace - currentFace % 1000 + currentFace % 100;
	return style + Math.floor(Math.random() * 8) * 100;
}

function getColorChoices(currentFace) {
	let style = currentFace - currentFace % 1000 + currentFace % 100;
	let colors = new Array(8);
	for (let i = 0; i < 8; i++)
		colors[i] = style + i * 100; //eye color choices use current eye style
	return colors;
}
...
if (selection == 0) {
	...
	if (selection == 1) {
		if (npc.playerHasItem(5152010, 1)) {
			npc.takeItem(5152010, 1);
			npc.setPlayerFace(getRandomColor(npc.getPlayerFace()));
	...
	let faces = getColorChoices(npc.getPlayerFace());
	...
	if (npc.playerHasItem(5152013, 1)) {
		npc.takeItem(5152013, 1);
